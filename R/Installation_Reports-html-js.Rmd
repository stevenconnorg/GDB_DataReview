...
Please see Table \@ref(#tab:cmissFDS) below for a complete listing of the AF SDSFIE standard Feature Datasets  missing from `r installationName`'s geodatabase, where applicable.  
...
if (nrow(df) > 0 ){
  DT::datatable(df, filter = 'top',caption = htmltools::tags$caption("Missing Feature Datasets"))-> tb2
  tb2
} else {
  sumrow<-  paste0("0 Total Missing Feature Datasets")
  kable(df, filter = 'top',caption = htmltools::tags$caption("Missing Feature Datasets")) -> tb2
  tb2
}
...
if (nrow(df) > 0 ){
   DT:::datatable(df, filter = 'top',caption = htmltools::tags$caption("Missing Feature Classes by Feature Dataset"))-> tb2
  tb2
...
Of the required Feature Datasets within the `r compName` schema included in `r installationName`'s geodatabase, `r nrow(MissingFCs)` of the required Feature Classes are not present. Table \@ref(tab:cmissFC) below gives a listing of all the Feature Classes missing, along with the associated Feature Dataset, where applicable.  
...
if (nrow(df) > 0 ){
  ...
  sumrow<-paste0(nrow," Total Missing Feature Classes")
     DT::datatable(df, filter = 'top',caption = htmltools::tags$caption("Missing Feature Classes"))-> tb2
  tb2
...
Of the standard Feature Classes included, `r nrow(MissingFLD)` standard fields for said Feature Classes in `r compName` schema are missing in `r installationName`'s geodatabase. Table \@ref(tab:cmissFLDincFC) : [link text](tab:cmissFLDincFC) gives a count of the *fields* missing from each standard Feature Class in `r installationName`'s geodatabase.  
...
For a more detailed breakdown of the individual missing fields from standard Feature Classes, see the '`r compName`_MissingData' table in `r installationName`'s geodatabase where rows are "T" for the "EMPTY_FC" column *and* rows are "0" for the "POP_VALS_COUNT" column.  
...
if (nrow(df) > 0 ){
  ...
  DT::datatable(df, filter = 'top',caption = htmltools::tags$caption("Missing Field   Counts by Feature Class"))-> tb2
...
```
Of the Feature Classes within the `r compName` schema included in `r installationName`'s geodatabase, `r Overview$InclFeatsEmpty` standard Feature Classes are empty. Table \@ref(tab:cemptFC2) gives a listing of all the empty Feature Classes in `r installationName`'s geodatabase. In total, `r sumEmptyFLD` empty Fields are present due to empty Feature Classes.  
...
if (nrow(df) > 0 ){
  ...
  DT::datatable(df, filter = 'top',caption = htmltools::tags$caption("Empty Feature Classes")) -> tb2
  tb2
} else {
...
if (nrow(df) > 0 ){
  ...
  sumrow<- paste0(sum," Total Empty Fields")
  DT::datatable(df, filter = 'top',caption = htmltools::tags$caption("Empty Fields from Non-Empty Feature Classes")) -> tb2
  tb2
} else {
  sumrow<- paste0("0 Total Empty Fields")
  DT::datatable(df, filter = 'top',caption = htmltools::tags$caption("Empty Fields from Non-Empty Feature Classes")) -> tb2
...
The `r Determinant_Values_by_FC[which.min(na.omit(Determinant_Values_by_FC)[,3]),2]` Feature class had the minimum percent determined percent with `r Determinant_Values_by_FC[which.min(na.omit(Determinant_Values_by_FC)[,3]),3]`% data determined, while the `r Determinant_Values_by_FC[which.max(na.omit(Determinant_Values_by_FC)[,3]),2]` Feature class had the minimum percent determined percent with `r Determinant_Values_by_FC[which.max(na.omit(Determinant_Values_by_FC)[,3]),3]`% data determined. On average, not including empty, standard Feature Classes, `r round(mean(Summary_Pct_by_FC$DETERMINED_PCT[!is.na(Summary_Pct_by_FC$DETERMINED_PCT)])*100,digits=1)`% of values are populated with determinant data for `r installationName`'s geodatabase across standard Feature Classes included.
...
if (nrow(df) > 0 ){
  DT::datatable(df, filter = 'top',caption = htmltools::tags$caption("Determinant Value Percentage by Feature Class")) -> tb2
  tb2
} else {
  DT::datatable(df, filter = 'top',caption = htmltools::tags$caption("Determinant Value Percentage by Feature Class")) -> tb2
  tb2
...
if (nrow(df) > 0 ){
DT::datatable(df, filter = 'top',caption = htmltools::tags$caption("Percent Other, TBD, and Null Cells by Feature Class"))  -> tb2
  tb2
} else {
DT::datatable(df, filter = 'top',caption = htmltools::tags$caption("Percent Other, TBD, and Null Cells by Feature Class"))  -> tb2
  tb2
...
## 'Null' Value Counts {#nullCnt}
 `r formatC(sum(MissingData$NULL_FC_COUNT), format="d", big.mark=",")`/`r formatC(sum(MissingData$POP_VALS_COUNT), format="d", big.mark=",")` (`r round(sum(MissingData$NULL_FC_COUNT) / (sum(MissingData$POP_VALS_COUNT))*100,digits=2)`%) cells within each standard Feature Class' Attribute Table exist within `r installationName`'s geodatabase. For the purposes of this report, 'Null' data is classified as any cell within a Feature Class' Attribute Table  that contains any one of the following values:
...
A more detailed breakdown of the individual values and counts of values for each *field* may be found in the `r installationName`'s geodatabase in the '`r compName`_MissingData' Table under the 'NULL_VALUE_COUNTS' column.  
...
if (nrow(df) > 0 ){
  ...
  DT::datatable(df, filter = 'top',caption = htmltools::tags$caption("Count of Null Cells by Feature Dataset")) -> tb2
  tb2
} else {
...
if (nrow(df) > 0 ){
  sum <- sum(df$Total_Null_Count)
  sumrow<- paste0(sum," Total 'Null' Cells")
  DT::datatable(df, filter = 'top',caption = htmltools::tags$caption("Count of Null Cells by Feature Class"))  -> tb2
  tb2
} else {
  sumrow<- paste0("0 Total 'Null' Cells")
  DT::datatable(df, filter = 'top',caption = htmltools::tags$caption("Count of Null Cells by Feature Class"))  -> tb2
...
A more detailed breakdown of the individual values and counts of values found for each *field* may be found in the `r installationName`'s geodatabase in the '`r compName`_MissingData' Table under the 'TBD_VALUE_COUNTS' column.  
...
if (nrow(df) > 0 ){
  ...
  DT::datatable(df, filter = 'top',caption = htmltools::tags$caption("Count of TBD Cells by Feature Dataset")) -> tb2
  tb2
} else {
...
if (nrow(df) > 0 ){
  sum<-sum(df$Total_TBD_Count)
  sumrow<-paste0(sum," Total 'TBD' Cells")
  DT::datatable(df, filter = 'top',caption = htmltools::tags$caption("Count of TBD Cells by Feature Class")) -> tb2
  tb2
} else {
  sumrow<-paste0("0 Total 'TBD' Cells")
  DT::datatable(df, filter = 'top',caption = htmltools::tags$caption("Count of TBD Cells by Feature Class")) -> tb2
...
For each standard Feature Class included in `r installationName`'s geodatabase, `r formatC(sum(MissingData$OTHER_FC_COUNT), format="d", big.mark=",")`/`r formatC(sum(MissingData$POP_VALS_COUNT), format="d", big.mark=",")` (`r round(sum(MissingData$OTHER_FC_COUNT) / (sum(MissingData$POP_VALS_COUNT))*100,digits=2)`)% cells within each Feature Class' Attribute Table exist.  
...
A more detailed breakdown of the individual values and counts of values for each *field* may be found in the `r installationName`'s geodatabase in the '`r compName`_MissingData' Table under the 'OTHER_VALUE_COUNTS' column.  
...
if (nrow(df) > 0 ){
  ...
  DT::datatable(df, filter = 'top',caption = htmltools::tags$caption("Count of Other Cells by Feature Dataset"))  -> tb2
  tb2
} else {
...
if (nrow(df) > 0 ){
  sum<-sum(df$Total_Other_Count)
  sumrow<- paste0(sum," Total 'Other' Cells")
  DT::datatable(df, filter = 'top',caption = htmltools::tags$caption("Count of Other Cells by Feature Class"))    -> tb2
  tb2
} else {
  sumrow<- paste0("0 Total 'Other' Cells")
DT::datatable(df, filter = 'top',caption = htmltools::tags$caption("Count of Other Cells by Feature Class"))    -> tb2
...
if (nrow(df) > 0 ){
  ...
  DT::datatable(df, filter = 'top',caption = htmltools::tags$caption("Count of Incorrectly Populated Domains by Feature Class")) -> tb2
  tb2
} else {
DT::datatable(df, filter = 'top',caption = htmltools::tags$caption("Count of Incorrectly Populated Domains by Feature Class")) -> tb2
}
...
In general, across AF Installations, if the Feature Class was included, Non-standard fields derived largely from fields such as "LAST_EDITED_DATE", "LAST_EDITED_USER", or "CREATED_DATE"; however, for a detailed listing of the individual *fields* included in `r installationName`'s geodatabase for each Feature Class in the `r compName` geodatabase, see the '`r compName`_MissingData' Table under the 'NON_SDS' column. Fields that are not included the `r compName` geodatabase will have the 'NON_SDS' column equal to 'T' (for 'True').  
...
if (nrow(df) > 0 ){
  ...
  sumrow<- paste0(sum," Total Count of Fields")
  DT::datatable(df, filter = 'top',caption = htmltools::tags$caption("Count of Fields not in Target Geodatabase by Feature Dataset"))  -> tb2
  tb2
} else {
 DT::datatable(df, filter = 'top',caption = htmltools::tags$caption("Count of Fields not in Target Geodatabase by Feature Dataset"))  -> tb2
  tb2
...
if (nrow(df) > 0 ){
  sum <- sum(df$NON_SDS_FIELD_COUNT)
  sumrow<- paste0(sum," Total Count of Fields")
  DT::datatable(df, filter = 'top',caption = htmltools::tags$caption("Count of Fields not in Target Geodatabase by Feature Class")) -> tb2
  tb2
} else {
  DT::datatable(df, filter = 'top',caption = htmltools::tags$caption("Count of Fields not in Target Geodatabase by Feature Class")) -> tb2
  tb2
