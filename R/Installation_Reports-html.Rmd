...
Upon receiving `r installationName`'s geodatabase, standard Feature Classes were migrated to standard Feature Datasets to match the `r compName` AF SDSFIE 3.101 geodatabase, where required.  

This report first lists which AF SDSFIE standard Feature Datasets are missing from `r installationName`'s geodatabase in comparison with the `r compName` geodatabase schema. This data can be found in the  Section \@ref(missFDS).

Then, for each of the AF SDSFIE standard Feature Datasets in the `r compName` geodatabase schema that are included in `r installationName`'s geodatabase, this report lists which AF SDSFIE standard Feature Classes are missing from those Feature Datasets (Sections \@ref(missFCFDS) - \@ref(missFC)). Section \@ref(missFLDincFC) continues to identify the counts of Missing Fields per standard Feature Classes included. Of the included Feature Classes, empty Feature Classes are listed in the 'Empty Feature Classes' section (Section \@ref(emptFC)).  Further, Section \@ref(emptFLDneFC) identifies the counts of empty fields from non-empty, standard Feature Classes included in `r installationName`'s geodatabase.

Then, for each of the AF SDSFIE standard Feature Dataset/Feature Class combinations from `r compName` schema that are included in `r installationName`'s geodatabase, this report then analyzes each AF SDSFIE standard Feature Class within `r installationName`'s geodatabase for indeterminant data at the Attribute level (Section \@ref(detindtVal)). For the purposes of this report, any data classified as 'Null', 'To be determined' (TBD), or 'Other' is considered 'indeterminant.' Section \@ref(pctindtFC) gives an overview of the percent of Attribute Table cells identified as 'Null', 'TBD', and 'Other' per Feature Class. The specific values that comprise 'Null', 'TBD', and 'Other' data are explicated in Sections \@ref(nullCnt), \@ref(tbdCnt), and \@ref(otherCnt), respectively, which give the counts of these indeterminent values by Feature Dataset and Feature Class within its subsections.

Finally, this report identifies noted discrepencies between the target `r compName` schema and `r installationName`'s geodatabase (Section \@ref(diff)) for  (1) Fields with Incorrectly Populated Domains (Section \@ref(diffIncDom)) and (2) Fields Included in `r installationName`'s Geodatabase Not in the `r compName` schema (Section \@ref(diffFLD)).

The data utilized to produce this report may be found within `r installationName`'s geodatabase in tables prepended with '`r compName`' (e.g.: '`r compName`_MissingData'). Specific references to these tables are found throughout the report where further details may be warranted.
...
6. **`r round(mean(Summary_Pct_by_FC$DETERMINED_PCT[!is.na(Summary_Pct_by_FC$DETERMINED_PCT)])*100,digits=1)`%** : Average Percent of Determinant Data across included standard Feature Classes  
...
if (nrow(df) > 0 ){
  df %>% kable("html", align = "c", linesep = "",caption="Missing Feature Datasets") %>% 
  
   kable_styling(bootstrap_options = c("striped", "hover")) %>%
    footnote(general= c(sumrow)) -> tb2
  tb2
} else {
  df %>% kable("html",caption="Missing Feature Datasets") %>% 
   kable_styling(bootstrap_options = c("striped", "hover")) %>%
    footnote(general= c(sumrow)) -> tb2
  tb2
}
...
if (nrow(df) > 0 ){
  df %>% kable("html", linesep = "", align = "c",  caption="Missing Feature Classes by Feature Dataset")  %>% 
  ...
  df %>% kable("html",caption="Missing Feature Classes by FDS")  %>% 
   kable_styling(bootstrap_options = c("striped", "hover")) %>%
    footnote(general= c(sumrow))  -> tb2
  tb2
}
...
if (nrow(df) > 0 ){
  ...
  df %>% kable("html",linesep = "", align = "c",  caption = "Missing Feature Classes")  %>% 
  ...
  df %>% kable("html", caption = "Missing Feature Classes")  %>% 
   kable_styling(bootstrap_options = c("striped", "hover")) %>%
    footnote(general= c(sumrow)) -> tb2
  tb2
}
...
Of the standard Feature Classes included, `r nrow(MissingFLD)` standard fields for said Feature Classes in `r compName` schema are missing in `r installationName`'s geodatabase. Table \@ref(tab:cmissFLDincFC) gives a count of the *fields* missing from each standard Feature Class in `r installationName`'s geodatabase.  
...
For a more detailed breakdown of the individual missing fields from standard Feature Classes, see the '`r compName`_MissingFields' table in `r installationName`'s geodatabase.  
...
if (nrow(df) > 0 ){
  ...
  df %>% kable("html", linesep = "", align = "c",   caption = "Missing Field Counts by Feature Class")  %>%
  ...
  df %>% kable("html", caption = "Missing Field Counts by Feature Class")  %>% 
   kable_styling(bootstrap_options = c("striped", "hover")) %>%
    footnote(general= c(sumrow)) -> tb2
  tb2
}
...
if (nrow(df) > 0 ){
  ...
  df %>% kable("html", linesep = "",  align = "c", caption = "Empty Feature Classes")%>%
  ...
  df %>% kable("html",caption = "Empty Feature Classes") %>% 
   kable_styling(bootstrap_options = c("striped", "hover")) %>%
    footnote(general= c(sumrow))-> tb2
  tb2
}
...
For a more detailed breakdown of the individual empty fields from non-empty feature classes, see the '`r compName`_MissingData' table in `r installationName`'s geodatabase where rows are 'F' for the 'EMPTY_FC' column *and* rows are '0' for the 'POP_VALS_COUNT' column.  
...
if (nrow(df) > 0 ){
  ...
  sumrow<- paste0(sum," Total Empty Fields")
  df %>% kable("html",linesep = "",  align = "c", caption = "Empty Fields from Non-Empty Feature Classes")%>% 
  ...
  sumrow<- paste0("0 Total Empty Fields")
  df %>% kable("html",caption = "Empty Fields from Non-Empty Feature Classes")%>% 
   kable_styling(bootstrap_options = c("striped", "hover")) %>%
    footnote(general= c(sumrow))  -> tb2
  tb2
}
...
Within the standard Feature Dataset/Feature Class combinations included in `r installationName`'s geodatabase, the percentage of data populated with determinant values (i.e.: data **not** classified as 'Null', 'TBD', or 'Other') are recorded in Table \@ref(tab:cdetindtVal2) below by Feature Class.  

The '`r Determinant_Values_by_FC[which.min(Determinant_Values_by_FC[,"PERCENT_DETERMINED_VALUES"]),"FC"]`' Feature class had the minimum data determined percentage of `r round(min(Determinant_Values_by_FC$PERCENT_DETERMINED_VALUES[!is.na(Determinant_Values_by_FC$PERCENT_DETERMINED_VALUES)])*100,digits=1)`%, while the '`r Determinant_Values_by_FC[which.max(Determinant_Values_by_FC[,"PERCENT_DETERMINED_VALUES"]),"FC"]`' Feature class had the maximum data determined percentage of `r round(max(Determinant_Values_by_FC$PERCENT_DETERMINED_VALUES[!is.na(Determinant_Values_by_FC$PERCENT_DETERMINED_VALUES)])*100,digits=1)`%. On average, not including empty, standard Feature Classes, `r round(mean(Summary_Pct_by_FC$DETERMINED_PCT[!is.na(Summary_Pct_by_FC$DETERMINED_PCT)])*100,digits=1)`% of values are populated with determinant data for `r installationName`'s geodatabase across standard Feature Classes included. This information is also found in `r installationName`'s geodatabase under the '`r compName`_Determinant_Values_by_FC' table. 
...
if (nrow(df) > 0 ){
  df %>% kable("html",linesep = "",  align = "c",  format.args = list(decimal.mark = ".", big.mark = ","),caption = "Determinant Value Percentage by Feature Class") %>% 
   kable_styling(bootstrap_options = c("striped", "hover"))  -> tb2
  tb2
} else {
  df %>% kable("html", format.args = list(decimal.mark = ".", big.mark = ","),caption = "Determinant Value Percentage by Feature Class") %>% 
   kable_styling(bootstrap_options = c("striped", "hover")) -> tb2
  tb2
}
...
if (nrow(df) > 0 ){
  df %>% kable("html",linesep = "",  align = "c",  format.args = list(decimal.mark = ".", big.mark = ","),caption="Percent Other, TBD, and Null Cells by Feature Class") %>% 

  
   kable_styling(bootstrap_options = c("striped", "hover"))   -> tb2
  tb2
} else {
  df %>% kable("html", format.args = list(decimal.mark = ".", big.mark = ","),caption="Percent Other, TBD, and Null Cells by Feature Class") %>% 
   kable_styling(bootstrap_options = c("striped", "hover"))  -> tb2
  tb2
}
...
if (nrow(df) > 0 ){
  ...
  df %>% kable("html", linesep = "",  align = "c",  format.args = list(decimal.mark = ".", big.mark = ","),caption="Count of Null Cells by Feature Dataset") %>% 
  ...
  df %>% kable("html", format.args = list(decimal.mark = ".", big.mark = ","),caption="Count of Null Cells by Feature Dataset") %>% 
   kable_styling(bootstrap_options = c("striped", "hover"))%>%
    footnote(general= c(sumrow))-> tb2
  tb2
}
...
if (nrow(df) > 0 ){
  sum <- sum(df$Total_Null_Count)
  sumrow<- paste0(sum," Total 'Null' Cells")
  df %>% kable("html", linesep = "",  align = "c",  format.args = list(decimal.mark = ".", big.mark = ","),caption="Count of Null Cells by Feature Class")  %>% 
   kable_styling(bootstrap_options = c("striped", "hover"))  %>%
  ...
  sumrow<- paste0("0 Total 'Null' Cells")
  df %>% kable("html", format.args = list(decimal.mark = ".", big.mark = ","),caption="Count of Null Cells by Feature Class")  %>% 
   kable_styling(bootstrap_options = c("striped", "hover")) %>%
    footnote(general= c(sumrow))-> tb2
  tb2
}
...
A more detailed breakdown of the individual values and counts of values found for each *field* may be found in the `r installationName`'s geodatabase in the '`r compName`_MissingData' Table under the 'TBD_VALUE_COUNTS' column. Further, you may see the counts of 'Null' cells by standard Feature Class in the '`r compName`_TBDCellCountbyFC' Table, or by Field in the '`r compName`_TBDCellCountbyFLD' Table, in `r installationName`'s geodatabase. 
...
if (nrow(df) > 0 ){
  ...
  df %>% kable("html", linesep = "",  align = "c",  format.args = list(decimal.mark = ".", big.mark = ","),caption="Count of TBD Cells by Feature Dataset") %>% 
   kable_styling(bootstrap_options = c("striped", "hover"))%>%
  ...
  df %>% kable("html", format.args = list(decimal.mark = ".", big.mark = ","),caption="Count of TBD Cells by Feature Dataset") %>% 
   kable_styling(bootstrap_options = c("striped", "hover"))%>%
    footnote(general= c(sumrow))-> tb2
  tb2
}
...
if (nrow(df) > 0 ){
  sum<-sum(df$Total_TBD_Count)
  sumrow<-paste0(sum," Total 'TBD' Cells")
  df %>% kable("html", linesep = "",  align = "c",  format.args = list(decimal.mark = ".", big.mark = ","),caption="Count of TBD Cells by Feature Class")  %>% 
   kable_styling(bootstrap_options = c("striped", "hover")) %>%
  ...
  sumrow<-paste0("0 Total 'TBD' Cells")
  df %>% kable("html", format.args = list(decimal.mark = ".", big.mark = ","),caption="Count of TBD Cells by Feature Class")  %>% 
   kable_styling(bootstrap_options = c("striped", "hover")) %>%
    footnote(general= c(sumrow)) -> tb2
  tb2
}
...
A more detailed breakdown of the individual values and counts of values for each *field* may be found in the `r installationName`'s geodatabase in the '`r compName`_MissingData' Table under the 'OTHER_VALUE_COUNTS' column.   Further, you may see the counts of 'Null' cells by standard Feature Class in the '`r compName`_OtherCellCountbyFC' Table, or by Field in the '`r compName`_OtherCellCountbyFLD' Table, in `r installationName`'s geodatabase.
...
if (nrow(df) > 0 ){
  ...
  df %>% kable("html", linesep = "",  align = "c",  format.args = list(decimal.mark = ".", big.mark = ","),caption="Count of Other Cells by Feature Dataset") %>% 
   kable_styling(bootstrap_options = c("striped", "hover")) %>%
  ...
  df %>% kable("html", format.args = list(decimal.mark = ".", big.mark = ","),caption="Count of Other Cells by Feature Dataset") %>% 
   kable_styling(bootstrap_options = c("striped", "hover"))%>%
    footnote(general= c(sumrow))-> tb2
  tb2
}
...
if (nrow(df) > 0 ){
  sum<-sum(df$Total_Other_Count)
  sumrow<- paste0(sum," Total 'Other' Cells")
  df %>% kable("html", linesep = "",  align = "c",  format.args = list(decimal.mark = ".", big.mark = ","),caption="Count of Other Cells by Feature Class")  %>% 
   kable_styling(bootstrap_options = c("striped", "hover"))%>%
  ...
  sumrow<- paste0("0 Total 'Other' Cells")
  df %>% kable("html", format.args = list(decimal.mark = ".", big.mark = ","),caption="Count of Other Cells by Feature Class")  %>% 
   kable_styling(bootstrap_options = c("striped", "hover")) %>%
    footnote(general= c(sumrow)) -> tb2
  tb2
}
...
Within the standard Feature Classes included in `r installationName`'s geodatabaase from the `r compName` schema, this section identifies the count of domain-contrained fields that have values outside of the domain values. In total, `r sum(incPopCountbyFC$Incorrectly_Populated_Field_Count)` fields have values that are not included in a field's domain.  

Please see Table \@ref(tab:cdiffIncDom2) below for the counts of incorrectly populated domains/fields by feature class.


A more detailed breakdown of the individual incorrectly-populated values and counts of these values may be found in the `r installationName`'s geodatabase within the '`r compName`_MissingData' Table under the 'INC_POP_VALS' column.   
...
if (nrow(df) > 0 ){
  ...
  df %>% kable("html",linesep = "",  align = "c",  format.args = list(decimal.mark = ".", big.mark = ","),caption="Count of Incorrectly Populated Domains by Feature Class")  %>% 
   kable_styling(bootstrap_options = c("striped", "hover"))%>%
  ...
  df %>% kable("html", format.args = list(decimal.mark = ".", big.mark = ","),caption="Count of Incorrectly Populated Domains by Feature Class")  %>% 
   kable_styling(bootstrap_options = c("striped", "hover"))%>%
    footnote(general= c(sumrow)) -> tb2
  tb2
}
...
In general, across AF Installations, if the Feature Class was included, Non-standard fields derived largely from fields such as "LAST_EDITED_DATE", "LAST_EDITED_USER", or "CREATED_DATE", therefore it is recommended that you cross-reference this sections with Section \@ref(missFLDincFC), where applicable. Further, for a more detailed listing of the individual non-standard *fields* included in `r installationName`'s geodatabase, see the '`r compName`_MissingData' Table under the 'NON_SDS' column. Fields that are not included the `r compName` geodatabase will have the 'NON_SDS' column equal to 'T' (for 'True').  
...
if (nrow(df) > 0 ){
  ...
  sumrow<- paste0(sum," Total Count of Fields")
  df %>% kable("html",   align = "c", caption="Count of Fields not in Target Geodatabase by Feature Dataset")  %>% 
   kable_styling(bootstrap_options = c("striped", "hover")) %>%
  ...
  sumrow<- paste0("0 Total Count of Fields")
   df %>% kable("html",caption="Count of Fields not in Target Geodatabase  by Feature Dataset")  %>% 
   kable_styling(bootstrap_options = c("striped", "hover")) %>%
    footnote(general= c(sumrow)) -> tb2
  tb2
}
...
if (nrow(df) > 0 ){
  sum <- sum(df$NON_SDS_FIELD_COUNT)
  sumrow<- paste0(sum," Total Count of Fields")
  df %>% kable("html", align = "c", caption="Count of Fields not in Target Geodatabase by Feature Class")  %>% 
   kable_styling(bootstrap_options = c("striped", "hover")) %>%
  ...
  sumrow<- paste0("0 Total Count of Fields")
   df %>% kable("html",caption="Count of Fields not in Target Geodatabase by Feature Class")  %>% 
   kable_styling(bootstrap_options = c("striped", "hover"))%>%
    footnote(general= c(sumrow))  -> tb2
  tb2
}
