...
# =============================================================================
	...
	### DeleteIdentical_management requires Advanced License !! rewrite with pandas
    #arcpy.DeleteIdentical_management(missFDSTable,fields=columns_to_check)
    missFDS_pd = table_to_pandas_dataframe(missFDSTable)
    missFDS_pd = missFDS_pd.drop_duplicates()
    arcpy.Delete_management(missFDSTable)
    pandas_to_table(missFDS_pd,missingFDTblName)

    columns_to_check=['FDS','FC_MISSING','INSTALLATION']
    #arcpy.DeleteIdentical_management(missFCTable,fields=columns_to_check)
    missFC_pd = table_to_pandas_dataframe(missFCTable)
    missFC_pd = missFC_pd.drop_duplicates()
    arcpy.Delete_management(missFCTable)
    pandas_to_table(missFC_pd,missingFCTblName)

    columns_to_check=['FDS','INSTALLATION','FC','FIELD_MISSING']
    #arcpy.DeleteIdentical_management(missFLDTable,fields=columns_to_check)
    missFLD_pd = table_to_pandas_dataframe(missFLDTable)
    missFLD_pd = missFLD_pd.drop_duplicates()
    arcpy.Delete_management(missFLDTable)
    pandas_to_table(missFLD_pd,missingFLDTblName)
